...
html,body { width:1080px; height:1350px; font-family:'Pretendard','Noto Sans KR','Noto Sans CJK KR',sans-serif; overflow:hidden;
...
/* 풀블리드 슬라이드(1,4)용 스크림 — 표지/마무리 텍스트가 어떤 사진 위에서도 확실히
   읽히도록, 텍스트가 놓이는 하단 60% 구간을 이전보다 더 일찍·더 진하게 어둡게 처리 */
.scrim-full { position:absolute; inset:0;
  background: linear-gradient(180deg, rgba(8,11,10,0.12) 0%, rgba(8,11,10,0.18) 25%, rgba(8,11,10,0.55) 48%, rgba(8,11,10,0.86) 68%, rgba(8,11,10,0.97) 100%);
}
...
""" + BASE_STYLE + """
.cover-content { position:absolute; left:56px; right:80px; top:840px; z-index:5; }
...
.term-heading { font-size:60px; font-weight:800; letter-spacing:-1.5px; margin-bottom:22px; }
.term-heading .accent-text { color:var(--accent); }
.definition { font-size:35px; line-height:1.5; font-weight:500; color:var(--panel-fg); margin-bottom:32px; letter-spacing:-0.3px; }
...
.step-value { font-size:26px; font-weight:700; color:var(--panel-fg); line-height:1.4; letter-spacing:-0.2px; }
.flow-note { margin-top:26px; font-size:20px; font-weight:600; color:var(--panel-muted); line-height:1.5; }
</style></head>
<body>
  <div class="card">
    ...
    <div class="panel panel-explain">
      ...
      <div class="term-heading">%%TERM_HEADING_HTML%%</div>
...
.compare-heading { font-size:50px; font-weight:800; letter-spacing:-1.3px; margin-bottom:14px; }
.compare-sub { font-size:26px; font-weight:500; line-height:1.48; color:var(--panel-muted); margin-bottom:28px; }

.chart-wrap { display:flex; align-items:flex-end; justify-content:center; gap:70px; margin-bottom:26px; }
.bar-col { display:flex; flex-direction:column; align-items:center; width:210px; }
.bar-value { font-size:29px; font-weight:800; color:var(--panel-fg); margin-bottom:12px; }
.bar { width:140px; border-radius:16px 16px 6px 6px; }
.bar-a { background: var(--accent); }
.bar-b { background: #9db3a3; }
.bar-label { margin-top:14px; font-size:19px; font-weight:800; letter-spacing:0.5px; color:var(--panel-muted); }

.callout { background:rgba(227,168,120,0.16); border-radius:16px; padding:24px 28px; font-size:26px; font-weight:600; line-height:1.5; color:var(--panel-fg); margin-bottom:16px; }
...
# ---------------------------------------------------------------------------
# 3-뉴스) 오늘의 경제 이슈 브리핑용 — 상단 사진 밴드 + 하단 패널
# (막대그래프 대신 "왜 중요할까" 임팩트 포인트 2개 + 한줄 요약 콜아웃 + 출처 표기)
# 실제 뉴스 기사를 요약하는 콘텐츠라 숫자를 시각화하는 막대그래프(SLIDE3_COMPARE)는
# 쓰지 않음 — 원문에 없는 수치를 그래프로 만들어 보여주는 것 자체가 사실 왜곡 위험이
# 있기 때문에, 사실 관계 대신 "의미/영향" 위주로 전달하는 이 템플릿을 씀.
# ---------------------------------------------------------------------------
SLIDE3_NEWS_IMPACT = """<!DOCTYPE html>
<html lang="ko"><head><meta charset="UTF-8"><style>
""" + BASE_STYLE + """
.photo-band { position:absolute; top:0; left:0; right:0; height:""" + str(PHOTO_BAND_HEIGHT) + """px; overflow:hidden; }
.panel-news { top:""" + str(PHOTO_BAND_HEIGHT) + """px; }

.impact-heading { font-size:50px; font-weight:800; letter-spacing:-1.3px; margin-bottom:28px; }

.impact-list { display:flex; flex-direction:column; gap:24px; margin-bottom:28px; }
.impact-row { display:flex; gap:20px; align-items:flex-start; }
.impact-badge { flex-shrink:0; width:48px; height:48px; border-radius:50%; background:var(--accent); color:#241a10;
  display:flex; align-items:center; justify-content:center; font-size:22px; font-weight:800; }
.impact-label { font-size:15px; font-weight:800; letter-spacing:1px; color:var(--panel-muted); margin-bottom:3px; }
.impact-value { font-size:27px; font-weight:700; color:var(--panel-fg); line-height:1.4; letter-spacing:-0.2px; }

.callout { background:rgba(227,168,120,0.16); border-radius:16px; padding:24px 28px; font-size:26px; font-weight:600; line-height:1.5; color:var(--panel-fg); margin-bottom:16px; }
.source-line { font-size:14px; color:var(--panel-muted); text-align:right; margin-top:auto; }
</style></head>
<body>
  <div class="card">
    <div class="photo-band">
      <img class="photo-bg" src="%%NEWS_IMPACT_BG%%">
      <div class="scrim-band-bottom" style="top:""" + str(PHOTO_BAND_HEIGHT - 180) + """px;"></div>
      <div class="scrim-top"></div>
    </div>

    <div class="topbar">
      <div class="tag-pill">%%IMPACT_TAG%%</div>
      <div class="wordmark"><span class="well">well</span>LOG</div>
    </div>

    <div class="panel panel-news">
      <div class="impact-heading">%%IMPACT_HEADING%%</div>

      <div class="impact-list">
        <div class="impact-row">
          <div class="impact-badge">1</div>
          <div><div class="impact-label">%%IMPACT1_LABEL%%</div><div class="impact-value">%%IMPACT1_VALUE%%</div></div>
        </div>
        <div class="impact-row">
          <div class="impact-badge">2</div>
          <div><div class="impact-label">%%IMPACT2_LABEL%%</div><div class="impact-value">%%IMPACT2_VALUE%%</div></div>
        </div>
      </div>

      <div class="callout">%%TAKEAWAY_HTML%%</div>
      <div class="source-line">출처: %%SOURCE_NAME%% · 일반적인 뉴스 요약이며 투자 조언이 아닙니다</div>
    </div>
  </div>
</body></html>
"""
