...
TOOL_SCHEMA = {
    ...
    "input_schema": {
        ...
        "required": [
            ...
            "reveal_tag", "definition", "definition_highlight",
...
def fetch_unsplash_photo(query: str, save_path: str, fallback_query: str) -> dict:
    """Unsplash에서 query로 사진을 검색해 save_path에 저장.
    검색 결과가 없으면 fallback_query로 한 번 더 시도.
    반환값: {"photographer": ..., "photographer_url": ...} (실패 시 None 값들)
    """
    ...
    def search(q):
        resp = requests.get(
            ...
            headers=headers,
            params={"query": q, "per_page": 1, "orientation": "portrait", "content_filter": "high"},
            timeout=20,
        )
...
def mark(text: str, highlight: str) -> str:
    """본문(정의/콜아웃)용 강조 — 형광펜 마커 스타일(<span class="mark">)."""
    if highlight and highlight in text:
        return text.replace(highlight, f'<span class="mark">{highlight}</span>', 1)
    return text
...
async def render_slides(data: dict, out_dir: str) -> list:
    ...
    for key, query, filename in slide_queries:
        ...
        credit = fetch_unsplash_photo(query, path, FALLBACK_QUERIES[key])
    ...
    slide2 = render_html(templates.SLIDE2_EXPLAIN, {
        ...
        "DEFINITION_HTML": mark(data["definition"], data.get("definition_highlight", "")),
    ...
    slide3 = render_html(templates.SLIDE3_COMPARE, {
        ...
        "CALLOUT_HTML": mark(data["callout"], data.get("callout_highlight", "")),
...
def main():
    ...
    credits = asyncio.run(render_slides(data, images_dir))
